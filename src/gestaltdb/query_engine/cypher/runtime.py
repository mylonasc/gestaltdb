...
from .plan import Distinct as LogicalDistinct
from .plan import FilterExpression as LogicalFilterExpression
from .plan import Limit as LogicalLimit
from .plan import Project as LogicalProject
from .plan import Skip as LogicalSkip
from .plan import Sort as LogicalSort
...
def _node_pattern_matches(node, pattern: NodePattern, properties, context: QueryContext) -> bool:
    ...
    An ``id`` entry always addresses the stable entity ID, in every pattern
    position; all other entries match stored properties.
    """
    ...
    for property_name, property_value in properties:
        if property_name == "id":
            if _cypher_equals(node.get_id, context.resolve(property_value)) is not True:
                return False
