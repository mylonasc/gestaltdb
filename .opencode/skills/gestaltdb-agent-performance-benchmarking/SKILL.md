...
description: Use when benchmarking how opencode agents and model configurations perform on GestaltDB agentic coding tasks, including token usage, tool usage, validation results, and HTML reports.
---

# GestaltDB Agent Performance Benchmarking

Use this skill to run repeatable agentic coding benchmarks against the GestaltDB repository. The benchmark runner starts isolated opencode runs in temporary git worktrees, measures speed, tokens, tool calls, deterministic validation results, and LLM-as-judge scores, then writes JSONL, CSV, summary JSON, and HTML reports.
...
- Validation stdout/stderr, session export, patch diff, and judge result.

Token and cost fields may be null for local providers that do not report usage.

## Benchmark Tasks

Task definitions live in `agentic-coding-efficiency-benchmarks/`. Each YAML file contains the coding prompt, validation commands, timeout, allowed file scopes, and judge rubric.

Initial tasks cover:

- Cypher inline property behavior.
- Typed sampling API usage.
- Deferred index maintenance behavior.
