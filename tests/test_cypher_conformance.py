...
UNSUPPORTED: list[tuple[str, str, str, str]] = [
    ...
    ("foreach", "[cypher-18]", "FOREACH (x IN [1] | CREATE (n))",
     "Unsupported Cypher query"),
    ("case-expression", "[cypher-04]", "MATCH (n) RETURN CASE WHEN n.age > 1 THEN 1 ELSE 0 END",
     "Unsupported Cypher query"),
    ("list-comprehension", "[cypher-04]", "MATCH (n) RETURN [x IN [1,2] WHERE x > 1 | x]",
     "Unsupported Cypher query"),
    ("map-projection", "[cypher-04]", "MATCH (n) RETURN n{.*, age: n.age}",
     "Unsupported Cypher query"),
    ("quantified-predicate", "[cypher-04]", "MATCH (n) RETURN all(x IN [1] WHERE x > 0)",
     "Unsupported Cypher query"),
    ("scalar-function", "[cypher-05]", "MATCH (n) RETURN toString(n.age)",
